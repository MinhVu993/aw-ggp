...
const newTranslations = `
  ...
  ITEMS: { vi: 'Vật tư', en: 'Items', zh: '物品' },
  DESTINATION: { vi: 'Nơi đến', en: 'Destination', zh: '目的地' },
  ITEMS_LIST: { vi: 'Danh sách vật tư', en: 'Items List', zh: '物品清单' },
...
if (content.includes('interface LanguageContextType')) {
    content = content.replace('};\\n\\ninterface LanguageContextType', newTranslations + '\\n};\\n\\ninterface LanguageContextType');
    // If it fails with literal \\n
    content = content.replace('};\\r\\n\\r\\ninterface LanguageContextType', newTranslations + '\\n};\\n\\ninterface LanguageContextType');
}

// More robust way: Find the exact place to insert (before `interface LanguageContextType`)
const parts = content.split('interface LanguageContextType');
if (parts.length > 1) {
  let before = parts[0];
  // remove the last `};`
  const lastBracketIndex = before.lastIndexOf('};');
  if (lastBracketIndex !== -1) {
    before = before.substring(0, lastBracketIndex) + newTranslations + '\\n};\\n\\n';
  }
  content = before + 'interface LanguageContextType' + parts[1];
}

fs.writeFileSync(contextPath, content, 'utf8');
console.log('Language context updated properly');
